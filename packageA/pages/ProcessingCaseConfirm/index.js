              ...
              success :(res)=> {
                res.data.rows.forEach((item,index)=>{
                   ...
                   item.FileExtension=='.gif'){
                    ...
                    that.data.videos2.push(item)
                    that.setData({
                      havefile2:true
                     })
                    
                   }else{
                     that.data.others2.push(item)
                     that.setData({
                      havefile2:true
                     })
                ...
                that.setData({
                  ...
                  videos2:that.data.videos2,
                  others2:that.data.others2
                })
              }
            })
        }
  ...
  submit:function(){
    ...
    Dialog.confirm({
      ...
      .then(() => {
       // on confirm
        wx.request({
          url: 'http://118.24.196.69:20015/api/HTSC_Case/IsCaseHandling', 
          method:'post',
          data: JSON.stringify({
            'suggestion':this.data.supervisesuggestion,
            'pid':that.data.editData.PID,
            'istype': this.data.statusType,
            }),
          async: true,
          header: {
            'content-type': 'application/json' // 默认值
          ...
          success :(res)=> {
           Notify({ type: 'success', message: '成功处理此案件！' });
            setTimeout(()=>{
                wx.navigateBack({
                  delta: 1,
                })
